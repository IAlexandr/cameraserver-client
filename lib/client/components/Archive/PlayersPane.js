...
export default class PlayersPane extends Component {
  ...
  prepCard ({player, cameracoder = {}}) {
    let playerPeriod = {};
    let cardStyle = {};
    if (this.props.archive.players[player.id]) {
      playerPeriod = this.props.archive.players[player.id].period;
    ...
    return (
      <Card style={{ padding: 5 }}>
        ...
        <PeriodPane
          ...
          period={playerPeriod}
          setPeriod={({period}) => {
            this.props.setPeriod({ period, cameraId: player.id });
          }}
          clearPeriod={(period) => {
            this.props.clearPeriod({ period, cameraId: player.id });
          }}
          togglePeriod={({isToggleOn}) => {
            this.props.togglePeriod({ isToggleOn, cameraId: player.id });
          }}
