...
import {camerasActions, monitorActions, smessagesActions} from './../../actions';
import Paper from 'material-ui/Paper';
import {settings} from './../../utils/settings';

// TODO разбить на мелкие компоненты, добавить фильтр по адресу.
class Cameras extends Component {
  ...
  playerToggle (camera) {
    ...
    if (this.props.monitor.players.hasOwnProperty(cameraId)) {
      ...
      if (Object.keys(this.props.monitor.players).length >= settings.MAX_PLAYERS_COUNT) {
      ...
      const definition = {
        columns: {
          'name': {
            ...
            order: 1,
            prepRowStyle: (obj) => {
              if (_this.props.monitor.players.hasOwnProperty(obj._id)) {
                return {
                  cursor: 'pointer',
                  color: 'green'
                };
        ...
        onCellClick (selectedIndex) {
          ...
          console.log('Добавить плеер для камеры', selectedIndex, camera);
...
export default connect(mapStateToProps, {
  ...
  addPlayer: monitorActions.addPlayer,
