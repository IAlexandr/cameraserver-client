...
class Monitor extends Component {
  ...
  static propTypes = {
    ...
    handleLeftPaneToggle: PropTypes.func,
  };

  prepPlayer ({ camera, width, height }) {
    let archivePeriod;
    if (!this.props.archive.multipleMode) {
      console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ', this.props.archive.players);
      if (this.props.archive.players[camera.id]) {
        archivePeriod = this.props.archive.players[camera.id].period;
      }
    } else {
      console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ');
  ...
  prepPlayers (players = {}) {
    ...
    if (cameraIds.length === 0) {
      return (
        <div style={{ paddingLeft: 'calc(50% - 100px)', width: 140, paddingTop: '25%', fontSize: 16 }}>
          <RaisedButton
            label="Выбор камер"
            labelPosition="before"
            style={{margin: 12, width: 200}}
            icon={<Videocam />}
            onClick={(() => this.props.handleLeftPaneToggle())}
          >
          </RaisedButton>
        </div>
      );
