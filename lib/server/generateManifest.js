...
function getAttr(props, mediaPresentationDuration) {
  ...
  if (!mediaPresentationDuration) {
    _attr = Object.assign(schema, {
      ...
      suggestedPresentationDelay,
      availabilityStartTime,
      publishTime: moment().subtract(30, 'seconds').utc().format('YYYY:MM:DDTHH:mm:ss'),
    });
  } else {
    _attr = Object.assign(schema, {
      type: 'static',
...
function getSegmentDuration(connection, expression, done) {
  connection.Segment.sum('duration', expression)
    .then(duration => {
      connection.Segment.count(expression)
        .then((count) => {
          const timeline = [];
          const median = duration / count;
          ...
          return done(null, timeline, duration);
        })
      ;
    })
  ;
...
function generateManifest({connection, sessionGuid, query = {}}, done) {
  connection.Session.findOne({sessionGuid})
    .then((session) => {
      ...
      if (live) {
        expression = {where: {sessionGuid, createdAt: {$lt: moment().subtract(30, 'seconds').valueOf()}}};
      } else {
      ...
      getSegmentDuration(connection, expression, (err, SegmentTimeline, duration) => {
        if (live) {
          attr = getAttr(props);
        } else {
          attr = getAttr(
